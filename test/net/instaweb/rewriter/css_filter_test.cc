 ...
 protected:
  void TestUrlAbsolutification(const StringPiece id,
    ...
    if (enable_mapping_and_sharding &&
      ...
      EXPECT_FALSE(
          domain_lawyer->DoDomainsServeSameContent("cdn.com", "test.com"));
      ASSERT_TRUE(domain_lawyer->AddRewriteDomainMapping(
          "http://cdn.com", "http://test.com", handler));
      EXPECT_TRUE(
          domain_lawyer->DoDomainsServeSameContent("cdn.com", "test.com"));
      ...
      EXPECT_TRUE(
          rewrite_driver()->ShouldAbsolutifyUrl(src_base, src_base, &proxying));
      EXPECT_FALSE(proxying);
      GoogleUrl dst_base("http://cdn.com/foo.css");
      proxying = true;  // again to ensure it's set to false.
      EXPECT_TRUE(
          rewrite_driver()->ShouldAbsolutifyUrl(src_base, dst_base, &proxying));
      EXPECT_FALSE(proxying);
    }
  ...
  void TestWebpRewriting(const char* input_image_name,
    ...
    GoogleString image_out = absl::StrFormat(
        *absl::ParsedFormat<'s'>::New(output_image_name_template),
        input_image_name);
    GoogleString css_input =
        absl::StrFormat("body{background:url(%s)}", input_image_name);
    GoogleString css_output =
        absl::StrFormat("body{background:url(%s)}", image_out.c_str());
    GoogleString expected_url = absl::StrFormat(
        *absl::ParsedFormat<'s'>::New(output_css_name_template), kInputCssName);
    AddFileToMockFetcher(StrCat(kTestDomain, input_image_name),
                         input_image_name, image_content_type, 100);

    SetResponseWithDefaultHeaders(kInputCssName, kContentTypeCss, css_input,
                                  100);
    ...
    GoogleString actual_output;
    EXPECT_TRUE(
        FetchResourceUrl(StrCat(kTestDomain, css_urls[0]), &actual_output));
...
TEST_F(CssFilterTestCustomOptions, CssPreserveUrlsWithMergedCacheExtend) {
  ...
  ValidateExpected("css_preserve_urls_on", "<link rel=StyleSheet href=a.css>",
...
TEST_F(CssFilterTest, LinkHrefCaseInsensitive) {
  ...
  SetResponseWithDefaultHeaders("a.css", kContentTypeCss, kInputStyle, 100);
  ValidateExpected("case_insensitive", "<link rel=StyleSheet href=a.css>",
                   StrCat("<link rel=StyleSheet href=",
                          ExpectedUrlForCss("a", kOutputStyle), ">"));
}
...
TEST_F(CssFilterTest, TransitionTimeUnits) {
  ...
  ValidateRewrite("transition_time", "foo {  -moz-transition-delay: 0s, 0s; }",
                  "foo{-moz-transition-delay:0s , 0s}", kExpectSuccess);
}
...
TEST_F(CssFilterTest, Non8BitEncoding) {
  ...
  ValidateRewrite("string-ma", ".foo { font-family: \"\x83\x7D\"; color: red }",
                  ".foo{font-family: \"\x83\x7D\";color:red}", kExpectSuccess);
  // 0x83 0x7B == KATAKANA LETTER BO
  // 0x7B == LEFT CURLY BRACKET {
  ValidateRewrite("string-bo", ".foo { font-family: \"\x83\x7B\"; color: red }",
                  ".foo{font-family: \"\x83\x7B\";color:red}", kExpectSuccess);
...
TEST_F(CssFilterTest, NoExtensionCorruption) { TestCorruptUrl(".css%22"); }

TEST_F(CssFilterTest, NoQueryCorruption) { TestCorruptUrl(".css?query"); }

TEST_F(CssFilterTest, NoWrongExtCorruption) { TestCorruptUrl(".html"); }
...
TEST_F(CssFilterTest, RewriteVariousCss) {
  ...
  const char* good_examples[] = {
      ...
      // Mismatched {}s that are acceptable because they do not fail to close
      // {s, but instead have stray }s that would not be parsed as closing
      // previous {s
      "a[}]{color:red}",
  ...
  const char* fail_examples[] = {
      ...
      // Trailing gibberish.
      // Note: we are depending upon these failing to indicate to
      // CssCombineFilter
...
TEST_F(CssFilterTest, ComplexCssTest) {
  ...
  const char* examples[][2] = {
      ...
      {"a { *padding-bottom: 0px; }", "a{*padding-bottom: 0px}"},
      ...
      // Parse and serialize "\n" correctly as "n" and "\A " correctly as
      // newline. But leave the original string without messing with escaping.
      ...
      {".authorization .mail .login input, .authorization .pswd input {"
       ...
       "#width: 88%; font-size: 75%; height: 21px; -moz-box-sizing: "
       "border-box; "
      ...
      {".login-form .input-text{ width:144px;padding:6px 3px; "
       ...
       "td.pop_content .dialog_body{padding:10px;border-bottom:1px# solid "
       "#ccc}",
      ...
      // @font-face
      // TODO(sligocki): src: attributes with multiple attributes are not
      // parsed.
      ...
      {"@media screen and (-webkit-min-device-pixel-ratio:0) {\n"
       ...
       "    src: url('fonts/tiefontello.svg?88026028#fontello') "
       "format('svg');\n"
       "  }\n"
       "}\n",
      ...
      // ::- in selectors
      {"::-moz-selection {background: #f36921 ; color: #fff ; "
       "text-shadow:none}",
...
TEST_F(CssFilterTest, NoAlwaysRewriteCss) {
  ...
  DebugWithMessage("");
  server_context()->ComputeSignature(options());
  ValidateRewrite("expanding_example", "@import 'http://www.example.com';",
                  "@import url(http://www.example.com);", kExpectSuccess);
  ...
  server_context()->ComputeSignature(options());
  ValidateRewrite("non_expanding_example", "@import 'http://www.example.com';",
                  "@import 'http://www.example.com';", kExpectNoChange);
...
TEST_F(CssFilterTest, RewriteStyleAttributeDifferentDirsNoUrl) {
  ...
  ValidateExpected("main_page", StrCat("<div style='", kCss, "'/>"),
                   StrCat("<div style='", kMinCss, "'/>"));

  ValidateExpected("subdir/file", StrCat("<div style='", kCss, "'/>"),
                   StrCat("<div style='", kMinCss, "'/>"));
...
TEST_F(CssFilterTest, DontAbsolutifyCssImportUrls) {
  ...
  const GoogleString css_in =
      StrCat("@import url(media/print.css) print;",
             "@import url(media/screen.css) screen;", styles_css);
...
TEST_F(CssFilterTest, WebpRewriting) {
  ...
  TestWebpRewriting(kPuzzleJpgFile, kContentTypeJpeg, "x%s.pagespeed.ic.0.webp",
...
TEST_F(CssFilterTest, NoWebpRewritingFromJpgIfDisabled) {
  ...
  TestWebpRewriting(kPuzzleJpgFile, kContentTypeJpeg, "x%s.pagespeed.ic.0.jpg",
                    "A.%s.pagespeed.cf.0.css");
}
...
TEST_F(CssFilterTest, NoWebpRewritingFromPngIfDisabled) {
  ...
  TestWebpRewriting(kBikePngFile, kContentTypePng, "x%s.pagespeed.ic.0.png",
                    "A.%s.pagespeed.cf.0.css");
}
...
TEST_F(CssFilterTest, WebpRewritingFromPngWithWebpFlagWebpLaUa) {
  ...
  TestWebpRewriting(kBikePngFile, kContentTypePng, "x%s.pagespeed.ic.0.webp",
                    "A.%s.pagespeed.cf.0.css");
}
...
TEST_F(CssFilterTest, DontAbsolutifyUrlsIfNoDomainMapping) {
  ...
  TestUrlAbsolutification(
      "dont_absolutify_unparseable_urls_etc_with", css_input, css_input,
      true /* expect_unparseable_section */, true /* enable_image_rewriting */,
      false /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
  // without image rewriting
  TestUrlAbsolutification(
      "dont_absolutify_unparseable_urls_etc_without", css_input, css_input,
...
TEST_F(CssFilterTest, AbsolutifyUnparseableUrlsWithDomainMapping) {
  ...
  // with image rewriting
  TestUrlAbsolutification(
      "absolutify_unparseable_urls_etc_with", css_input, css_output,
      true /* expect_unparseable_section */, true /* enable_image_rewriting */,
...
TEST_F(CssFilterTest, DontAbsolutifyCursorUrlsWithoutDomainMapping) {
  ...
  // with image rewriting
  TestUrlAbsolutification(
      "dont_absolutify_cursor_urls_etc_with", css_input, expected_output,
      false /* expect_unparseable_section */, true /* enable_image_rewriting */,
      false /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
  // without image rewriting
  TestUrlAbsolutification(
      "dont_absolutify_cursor_urls_etc_without", css_input, expected_output,
      false /* expect_unparseable_section */,
      false /* enable_image_rewriting */, false /* enable_proxy_mode */,
...
TEST_F(CssFilterTest, AbsolutifyCursorUrlsWithDomainMapping) {
  ...
  TestUrlAbsolutification(
      "absolutify_cursor_urls_with_domain_mapping", css_input, expected_output,
      false /* expect_unparseable_section */, true /* enable_image_rewriting */,
      false /* enable_proxy_mode */, true /* enable_mapping_and_sharding */);
}
...
TEST_F(CssFilterTest, EmptyLeafFetch) {
  // CSS URL ends in /
  SetResponseWithDefaultHeaders(StrCat(kTestDomain, "style/"), kContentTypeCss,
                                kInputStyle, 100);
...
TEST_F(CssFilterTest, UnauthorizedCssResource) {
  ...
  DebugWithMessage(
      StrCat("<!--",
             ...
             "-->"));
  ValidateRewriteExternalCssUrl("unauth", gurl.Spec(), kInputStyle, kInputStyle,
                                kExpectNoChange);
}
...
TEST_F(CssFilterTest, InlineCssWithExternalUrlAndDelayCache) {
  ...
  EXPECT_EQ(
      "<html><body><style>body{background:url(a.jpg)}</style>"
...
TEST_F(CssFilterTest, AlternateStylesheet) {
  ...
  ValidateExpected(
      ...
      absl::StrFormat(html_format, "alternate stylesheet", new_url.c_str()));

  ValidateExpected(
      "alternate_stylesheet2",
      absl::StrFormat(html_format, " StyleSheet alterNATE  ", "foo.css"),
      absl::StrFormat(html_format, " StyleSheet alterNATE  ", new_url.c_str()));

  ValidateExpected(
      "alternate_stylesheet_and_more",
  ...
  ValidateNoChanges(
      "alternate_not_stylesheet",
      absl::StrFormat(html_format, "alternate snowflake", "foo.css"));
}
...
TEST_F(CssFilterTest, AbsolutifyServingFallback) {
  ...
  GoogleString url =
      Encode(kTestDomain, "cf", "0" /* wrong hash */, "foo.css", "css");
...
TEST_F(CssFilterTest, BasicCsp) {
  EnableDebug();
  SetResponseWithDefaultHeaders("styles/a.css", kContentTypeCss, kInputStyle,
                                100);
  SetResponseWithDefaultHeaders("uploads/sneaky.png", kContentTypeCss,
                                kInputStyle, 100);
  ...
  ValidateExpected(
      ...
      StrCat(kCsp, CssLinkHref(Encode("styles/", "cf", "0", "a.css", "css")),
             CssLinkHref("uploads/sneaky.png"),
...
TEST_F(CssFilterTest, InlineCsp) {
  ...
  ValidateExpected(
      "inline_css", StrCat(kCsp, kCss, kStyledDiv),
      StrCat(kCsp, kCss,
...
TEST_F(CssFilterTest, RenderCsp) {
  ...
  // No CSP -> fine.
  ValidateExpected("no_csp", CssLinkHref("styles/a.css"),
                   CssLinkHref(Encode("styles/", "cf", "0", "a.css", "css")));

  // CSP applied at render time, from cached result.
  ValidateExpected(
      "render_csp", StrCat(kCsp, CssLinkHref("styles/a.css")),
      StrCat(kCsp, CssLinkHref("styles/a.css"),
...
TEST_F(CssFilterTest, InlineCspIrrelevant) {
  ...
  ValidateExpected("inline_css", StrCat(kCsp, kCss, kStyledDiv),
...
TEST_F(CssFilterTestUrlNamer, AbsolutifyUnparseableUrls) {
  ...
  // with image rewriting
  TestUrlAbsolutification(
      "absolutify_unparseable_urls_with", css_input, expected_output,
      true /* expect_unparseable_section */, true /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
  // without image rewriting
  TestUrlAbsolutification(
      "do_absolutify_unparseable_urls_without", css_input, expected_output,
      true /* expect_unparseable_section */, false /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
}
...
TEST_F(CssFilterTestUrlNamer, AbsolutifyParseableUrls) {
  ...
  const char expected_output[] = "body{background:url(http://test.com/a.png)}";
  // with image rewriting
  TestUrlAbsolutification(
      "absolutify_parseable_urls_with", css_input, expected_output,
      false /* expect_unparseable_section */, true /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
  // without image rewriting
  TestUrlAbsolutification(
      "absolutify_parseable_urls_without", css_input, expected_output,
      false /* expect_unparseable_section */,
      false /* enable_image_rewriting */, true /* enable_proxy_mode */,
      false /* enable_mapping_and_sharding */);
}
...
TEST_F(CssFilterTestUrlNamer, AbsolutifyOtherUrlsWithProxy) {
  ...
  TestUrlAbsolutification(
      "absolutify_other_urls_with_proxy", css_input, expected_output,
      false /* expect_unparseable_section */, true /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
}
...
TEST_F(CssFilterTestUrlNamer, AbsolutifyWithBom) {
  ...
  // with image rewriting
  TestUrlAbsolutification(
      "absolutify_with_bom_with", css_input, expected_output,
      true /* expect_unparseable_section */, true /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
  // without image rewriting
  TestUrlAbsolutification(
      "do_absolutify_with_bom_without", css_input, expected_output,
      true /* expect_unparseable_section */, false /* enable_image_rewriting */,
      true /* enable_proxy_mode */, false /* enable_mapping_and_sharding */);
}
