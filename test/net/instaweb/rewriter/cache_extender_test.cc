...
class CacheExtenderTest : public RewriteTestBase {
 protected:
  ...
  CacheExtenderTest()
    ...
    num_cache_extended_ =
        statistics()->GetVariable(CacheExtender::kCacheExtensions);
  }
  ...
  void InitResource(StringPiece resource_name, const ContentType& content_type,
                    StringPiece content, int64 ttl_sec) {
  ...
  GoogleString GenerateHtml(const GoogleString& a, const GoogleString& b,
    ...
    return absl::StrFormat(kHtmlFormat, a.c_str(), a_debug.c_str(), b.c_str(),
                           b_debug.c_str(), c.c_str(), c_debug.c_str());
  }
  ...
  void TestExtendFromHtml() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (lru_cache()->IsHealthy()) {
        ...
        VerifyRewriterInfoEntry(log_record, "ec", 0, (i * 3), (i + 1) * 3, 3,
                                "http://test.com/sub/a.css?v=1");
  ...
  void TestServeFiles() {
    ...
    InitTest(kShortTtlSec);
    int64 expected_last_modified_time_ms =
        last_modified_on_origin_ ? timer()->NowMs() : 0;
  ...
  void VerifyUnauthorizedResourcesNotExtended() {
    ...
    ValidateExpected(
        "dont_extend_unauth_js", StrCat(kJsReference, kCssReference),
        StrCat(kJsReference, kDebugMessage, kCssReference, kDebugMessage));
    EXPECT_EQ(0, num_cache_extended_->Get())
        << "Number of cache extended resources is wrong";
    EXPECT_STREQ("", AppliedRewriterStringFromLog());
  }
 ...
 public:
  ...
  void TestExtend(bool img_extend, bool css_extend, bool js_extend) {
    ...
    ValidateExpected("do_extend",
                     GenerateHtml(kCssFile, "b.jpg", "c.js", kInput),
                     GenerateHtml(expected_css_html, expected_img_html,
                                  expected_js_html, kOutput));
  }
};
...
TEST_F(CacheExtenderTest, DontExtendIntrospectiveDefault) {
  InitTest(kShortTtlSec);
  const char kJsTemplate[] = "<script src=\"%s\"></script>";
  ValidateExpected("dont_extend_introspective_js",
...
TEST_F(CacheExtenderTest, DontExtendIntrospectiveDebug) {
...
TEST_F(CacheExtenderTest, DoExtendIntrospectiveJavascript) {
  ...
  ValidateExpected(
      ...
      absl::StrFormat(
          kJsTemplate,
          Encode("", kFilterId, "0", "introspective.js", "js").c_str()));
}
...
TEST_F(CacheExtenderTest, DoExtendLinkRelCaseInsensitive) {
  ...
  ValidateExpected(
      "extend_ci", absl::StrFormat(kMixedCaseTemplate, kCssFile),
      absl::StrFormat(
          kMixedCaseTemplate,
          Encode(kCssSubdir, kFilterId, "0", kCssTail, "css").c_str()));
}

TEST_F(CacheExtenderTest, DoExtendForImagesOnly) {
  AddFilter(RewriteOptions::kExtendCacheImages);
  SetResponseWithDefaultHeaders(kCssFile, kContentTypeCss, kCssData,
                                kShortTtlSec);
  SetResponseWithDefaultHeaders("b.jpg", kContentTypeJpeg, kImageData,
                                kShortTtlSec);
  SetResponseWithDefaultHeaders("c.js", kContentTypeJavascript, kJsData,
                                kShortTtlSec);

  for (int i = 0; i < 3; i++) {
    ValidateExpected(
        "do_extend", GenerateHtml(kCssFile, "b.jpg", "c.js", kInput),
        GenerateHtml(kCssFile, Encode("", kFilterId, "0", "b.jpg", "jpg"),
                     "c.js", kOutput));
...
TEST_F(CacheExtenderTest, UrlTooLong) {
  ...
  SetResponseWithDefaultHeaders(css_name, kContentTypeCss, kCssData,
                                kShortTtlSec);
  SetResponseWithDefaultHeaders(jpg_name, kContentTypeJpeg, kImageData,
                                kShortTtlSec);
  SetResponseWithDefaultHeaders(js_name, kContentTypeJavascript, kJsData,
                                kShortTtlSec);

  // If filename wasn't too long, this would be rewritten (like in DoExtend).
  ValidateNoChanges("url_too_long",
                    GenerateHtml(css_name, jpg_name, js_name, kBoth));
  EXPECT_EQ(0, num_cache_extended_->Get())
      << "Number of cache extended resources is wrong";
}
...
TEST_F(CacheExtenderTest, NoInputResource) {
  ...
  ValidateNoChanges("bad url", GenerateHtml("swly://example.com/sub/a.css",
                                            "http://evil.com/b.jpg",
...
TEST_F(CacheExtenderTest, ExtendIfSharded) {
  ...
  ValidateExpected(
      "extend_if_sharded", GenerateHtml(kCssFile, "b.jpg", "c.js", kOutput),
      GenerateHtml(Encode(StrCat("http://shard0.com/", kCssSubdir), kFilterId,
                          "0", kCssTail, "css"),
                   Encode("http://shard0.com/", kFilterId, "0", "b.jpg", "jpg"),
                   Encode("http://shard0.com/", kFilterId, "0", "c.js", "js"),
                   kOutput));
}
...
TEST_F(CacheExtenderTest, ExtendIfOriginMappedHttps) {
  ...
  ValidateExpected(
      ...
      GenerateHtml("https://cdn.com/sub/a.css?v=1", "https://cdn.com/b.jpg",
...
TEST_F(CacheExtenderTest, ExtendIfRewritten) {
  ...
  ValidateExpected(
      "extend_if_rewritten", GenerateHtml(kCssFile, "b.jpg", "c.js", kInput),
      GenerateHtml(
...
TEST_F(CacheExtenderTest, NoLinkCanonicalIfDomainMapComplexity) {
  ...
  GoogleString content;
  ASSERT_TRUE(FetchResourceUrl(
      Encode(kTestDomain, kFilterId, "0", "b.jpg", "jpg"), &content, &headers));
  EXPECT_FALSE(headers.Has(HttpAttributes::kLink));

  // Likewise for reconstruction.
  lru_cache()->Clear();
  ASSERT_TRUE(FetchResourceUrl(
      Encode("http://cdn.com/", kFilterId, "0", "b.jpg", "jpg"), &content,
      &headers));
  EXPECT_FALSE(headers.Has(HttpAttributes::kLink));
}
...
void CacheExtenderTest::TestCanonicalOnFallback(
  ...
  GoogleString out_css_url(Encode(StrCat(kTestDomain, kCssSubdir), kFilterId,
                                  hash, kCssTail, "css"));
  GoogleString out_jpeg_url(
      Encode(kTestDomain, kFilterId, hash, "b.jpg", "jpg"));
  ...
  EXPECT_STREQ(
      ResponseHeaders::RelCanonicalHeaderValue(StrCat(kTestDomain, "b.jpg")),
      headers.Lookup1(HttpAttributes::kLink));

  // Should also propagate existing ones (and not add incompatible ones).
  lru_cache()->Clear();
  AddToResponse(StrCat(kTestDomain, "b.jpg"), HttpAttributes::kLink,
  ...
  EXPECT_STREQ(
      ResponseHeaders::RelCanonicalHeaderValue(StrCat(kTestDomain, "notb.jpg")),
      headers.Lookup1(HttpAttributes::kLink));
}
...
TEST_F(CacheExtenderTest, ExtendIfShardedToHttps) {
  ...
  EXPECT_TRUE(
      AddShard("https://test.com", "https://shard0.com,https://shard1.com"));
  // shard0 is always selected in the test because of our mock hasher
  // that always returns 0.
  ValidateExpected(
      "extend_if_sharded_to_https",
      GenerateHtml("https://test.com/sub/a.css?v=1", "https://test.com/b.jpg",
...
TEST_F(CacheExtenderTest, ExtendIfShardedAndRewritingAndMappingHttps) {
  ...
  ASSERT_TRUE(
      AddShard("http://cdn.com", "http://s1.cdn.com,http://s2.cdn.com"));
  ASSERT_TRUE(
      AddShard("https://cdn.com", "https://s1.cdn.com,https://s2.cdn.com"));
  ASSERT_TRUE(AddOriginDomainMapping("http://test.com", "https://test.com"));
  ASSERT_TRUE(
      AddOriginDomainMapping("http://test.com", "https://www.test.com"));

  // shard0 is always selected in the test because of our mock hasher
  // that always returns 0.
  ValidateExpected(
      "extend_if_sharded_rewriting_mapping_bare_domain_http",
      GenerateHtml("http://test.com/sub/a.css?v=1", "http://test.com/b.jpg",
  ...
  ValidateExpected(
      ...
      GenerateHtml("http://www.test.com/sub/a.css?v=1",
                   "http://www.test.com/b.jpg", "http://www.test.com/c.js",
                   kInput),
      GenerateHtml(
          Encode("http://s1.cdn.com/sub/", kFilterId, "0", kCssTail, "css"),
          Encode("http://s1.cdn.com/", kFilterId, "0", "b.jpg", "jpg"),
  ...
  ValidateExpected(
      ...
      GenerateHtml("https://www.test.com/sub/a.css?v=1",
                   "https://www.test.com/b.jpg", "https://www.test.com/c.js",
                   kInput),
      GenerateHtml(
          Encode("https://s1.cdn.com/sub/", kFilterId, "0", kCssTail, "css"),
          Encode("https://s1.cdn.com/", kFilterId, "0", "b.jpg", "jpg"),
...
TEST_F(CacheExtenderTest, ConsistentHashWithRewrite) {
  ...
  ValidateExpected("consistent_hash", absl::StrFormat(kCssFormat, kCssFile),
...
TEST_F(CacheExtenderTest, ConsistentHashWithShard) {
  ...
  GoogleString extended_css =
      Encode("http://shard2.com/sub/", kFilterId, kHash, kCssTail, "css");
  ValidateExpected("consistent_hash", absl::StrFormat(kCssFormat, kCssFile),
                   absl::StrFormat(kCssFormat, extended_css.c_str()));
...
TEST_F(CacheExtenderTest, ServeFilesFromDelayedFetch) {
  ...
  ServeResourceFromManyContexts(
      Encode(kTestDomain, kFilterId, "0", "b.jpg", "jpg"), kImageData);
  ServeResourceFromManyContexts(
      Encode(kTestDomain, kFilterId, "0", "c.js", "js"), kJsData);
...
TEST_F(CacheExtenderTest, MinimizeCacheHits) {
  ...
  GoogleString html_output = absl::StrFormat(
      ...
      Encode(kTestDomain, CssOutlineFilter::kFilterId, "0", "_", "css")
          .c_str());
...
TEST_F(CacheExtenderTest, TrimUrlInteraction) {
  ...
  GoogleString a_ext = Encode(kCssSubdir, kFilterId, "0", kCssTail, "css");
  ValidateExpected("ce_then_trim", absl::StrFormat(kCssFormat, kCssFile),
...
TEST_F(CacheExtenderTest, DefangHtml) {
  ...
  SetResponseWithDefaultHeaders("a.html", kContentTypeHtml, "boo!",
                                kShortTtlSec);
  SetResponseWithDefaultHeaders("a.xhtml", kContentTypeXhtml, "bwahahaha!",
                                kShortTtlSec);
  SetResponseWithDefaultHeaders("a.xml", kContentTypeXml, "boo!", kShortTtlSec);

  const GoogleString css_before = StrCat(
      CssLinkHref("a.html"), CssLinkHref("a.xhtml"), CssLinkHref("a.xml"));
...
TEST_F(CacheExtenderTest, DoNotExtendRewrittenCss) {
  ...
  lawyer->AddRewriteDomainMapping(kRewriteDomain, kTestDomain,
                                  message_handler());
  lawyer->AddShard(kRewriteDomain, StrCat(kShard1Domain, ",", kShard2Domain),
                   message_handler());
  options()->EnableFilter(RewriteOptions::kRewriteCss);
  InitTest(kShortTtlSec);
  ValidateExpected(
      "do_not_extend_rewritten_css", absl::StrFormat(kCssFormat, kCssFile),
      absl::StrFormat(kCssFormat,
                      Encode(StrCat(kShard1Domain, kCssSubdir),
                             RewriteOptions::kCssFilterId, "0", kCssTail, "css")
                          .c_str()));
...
TEST_F(CacheExtenderTest, AlternateStylesheet) {
  ...
  const GoogleString new_url =
      Encode(kCssSubdir, kFilterId, "0", kCssTail, "css");
  ...
  ValidateExpected(
      ...
      absl::StrFormat(html_format, " StyleSheet alterNATE  ", kCssFile),
      absl::StrFormat(html_format, " StyleSheet alterNATE  ", new_url.c_str()));

  ValidateExpected(
      "alternate_stylesheet_and_more",
      absl::StrFormat(html_format, "  foo stylesheet alternate bar ", kCssFile),
...
TEST_F(CacheExtenderTest, PreserveUrlRelativity) {
  ...
  ValidateExpected("preserve_url_relativity", "<img src=b.jpg>",
...
TEST_F(CacheExtenderTest, SrcSet) {
  ...
  ValidateExpected("srcset", "<img src=a.jpg srcset=\"a.jpg 1x, b.jpg 2x\">",
                   StrCat("<img src=", a_url, " srcset=\"", a_url, " 1x, ",
                          b_url, " 2x\">"));
}
...
TEST_F(CacheExtenderTest, VaryOrigin) {
  ...
  ValidateExpected("vary origin", absl::StrFormat(kCssFormat, "a.css"),
