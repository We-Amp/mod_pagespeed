...
class PthreadCondvarTest : public CondvarTestBase {
 protected:
  ...
  void CreateHelper() override {
    pthread_create(&helper_thread_, nullptr, &HelperThread, this);
  }

  void FinishHelper() override { pthread_join(helper_thread_, nullptr); }
...
TEST_F(PthreadCondvarTest, BlindSignals) { BlindSignalsTest(); }
...
TEST_F(PthreadCondvarTest, TestPingPong) { PingPongTest(); }
...
TEST_F(PthreadCondvarTest, TestTimeout) { TimeoutTest(); }
...
TEST_F(PthreadCondvarTest, TimeoutPingPong) { TimeoutPingPongTest(); }
