...
class ImageOptimizerTest : public testing::Test {
 public:
  ImageOptimizerTest() : message_handler_(new NullMutex) {}
 ...
 protected:
  ...
  void RewriteAndVerifyImages(const char* dir, const TestingImageInfo* images,
                              int num_images, bool is_animated,
    ...
    for (size_t i = 0; i < num_images; ++i) {
      ...
      if (!is_animated) {
        ...
        EXPECT_TRUE(AnalyzeImage(rewritten_format, rewritten_image.data(),
                                 rewritten_image.length(), &rewritten_width,
                                 &rewritten_height, nullptr, nullptr, nullptr,
                                 nullptr, nullptr, nullptr, &message_handler_));
        ...
        if (!compare_to_rewritten_dimensions) {
          if (original_format == IMAGE_JPEG) {
            ...
            // bit-by-bit.
            DecodeAndCompareImages(original_format, original_image.data(),
                                   original_image.length(), rewritten_format,
                                   rewritten_image.data(),
        ...
        EXPECT_TRUE(pagespeed::image_compression::CompareAnimatedImages(
            StrCat(dir, images[i].file_name), rewritten_image,
            &message_handler_));
      }
    }
  }
...
TEST_F(ImageOptimizerTest, AllFormats) {
  ...
  RewriteAndVerifyImages(
      kPngSuiteGifTestDir, kGifImages, kGifImageCount, is_animated, options,
      requested_dimension, IMAGE_GIF, expected_success, IMAGE_WEBP,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);

  RewriteAndVerifyImages(
      kPngSuiteTestDir, kPngImages, kPngImageCount, is_animated, options,
      requested_dimension, IMAGE_PNG, expected_success, IMAGE_WEBP,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);

  expected_uses_lossy_format = true;
  RewriteAndVerifyImages(
      kJpegTestDir, kJpegImages, kJpegImageCount, is_animated, options,
      requested_dimension, IMAGE_JPEG, expected_success, IMAGE_WEBP,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);

  is_animated = true;
  expected_uses_lossy_format = false;
  RewriteAndVerifyImages(
      kGifTestDir, kAnimatedGifImages, kAnimatedGifImageCount, is_animated,
      options, requested_dimension, IMAGE_GIF, expected_success, IMAGE_WEBP,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);
}
...
TEST_F(ImageOptimizerTest, GifToPng) {
  ...
  RewriteAndVerifyImages(
      kPngSuiteGifTestDir, kGifImages, kGifImageCount, is_animated, options,
      requested_dimension, IMAGE_GIF, expected_success, IMAGE_PNG,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);
}
...
TEST_F(ImageOptimizerTest, PngToPng) {
  ...
  RewriteAndVerifyImages(
      kPngSuiteTestDir, kPngImages, kPngImageCount, is_animated, options,
      requested_dimension, IMAGE_PNG, expected_success, IMAGE_PNG,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);
}
...
TEST_F(ImageOptimizerTest, JpegToJpeg) {
  ...
  RewriteAndVerifyImages(
      kJpegTestDir, kJpegImages, kJpegImageCount, is_animated, options,
      requested_dimension, IMAGE_JPEG, expected_success, IMAGE_JPEG,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);
}
...
TEST_F(ImageOptimizerTest, InvalidImages) {
  ...
  RewriteAndVerifyImages(
      kPngSuiteTestDir, kInvalidImages, kInvalidImageCount, is_animated,
      options, requested_dimension, IMAGE_PNG, expected_success, IMAGE_UNKNOWN,
      compare_to_rewritten_dimensions, expected_uses_lossy_format);
}
...
TEST_F(ImageOptimizerTest, SingleUse) {
  GoogleString original_image;
  ASSERT_TRUE(
      ReadTestFileWithExt(kPngSuiteTestDir, "basi0g01.png", &original_image));

  // Try to optimize the image.
  GoogleString rewritten_image;
  ImageFormat rewritten_format;
  ImageOptimizer optimizer(&message_handler_);
  ASSERT_TRUE(optimizer.Optimize(StringPiece(original_image), &rewritten_image,
                                 &rewritten_format));
  EXPECT_DEATH(optimizer.Optimize(StringPiece(original_image), &rewritten_image,
                                  &rewritten_format),
...
TEST_F(ImageOptimizerTest, MustReduceBytes) {
  ...
  ASSERT_FALSE(optimizer.Optimize(StringPiece(original_image), &rewritten_image,
                                  &rewritten_format));
}
