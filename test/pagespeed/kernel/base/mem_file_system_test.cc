...
#include "test/pagespeed/kernel/base/file_system_test_base.h"
...
class MemFileSystemTest : public FileSystemTest {
 protected:
  ...
  int DefaultDirSize() const override { return 0; }
...
TEST_F(MemFileSystemTest, TestWriteRead) { TestWriteRead(); }

// Write a temp file, then read it.
TEST_F(MemFileSystemTest, TestTemp) { TestTemp(); }

// Write a temp file, close it, append to it, then read it.
TEST_F(MemFileSystemTest, TestAppend) { TestAppend(); }
...
TEST_F(MemFileSystemTest, TestExists) { TestExists(); }

// Create a file along with its directory which does not exist.
TEST_F(MemFileSystemTest, TestCreateFileInDir) { TestCreateFileInDir(); }

// Make a directory and check that files may be placed in it.
TEST_F(MemFileSystemTest, TestMakeDir) { TestMakeDir(); }

// Create a directory and verify removing it.
TEST_F(MemFileSystemTest, TestRemoveDir) { TestRemoveDir(); }

// We intentionally do not test TestIsDir, TestRecursivelyMakeDir*

TEST_F(MemFileSystemTest, TestListContents) { TestListContents(); }

TEST_F(MemFileSystemTest, TestAtime) { TestAtime(); }

TEST_F(MemFileSystemTest, TestMtime) { TestMtime(); }
...
TEST_F(MemFileSystemTest, MtimeAtimeAcrossRename) {
  ...
  ASSERT_TRUE(
      file_system()->WriteFileAtomic("my.file", "hello, world", &handler_));
...
TEST_F(MemFileSystemTest, TestDirInfo) { TestDirInfo(); }
...
TEST_F(MemFileSystemTest, TestSizeOld) {
  ...
  ASSERT_TRUE(file_system()->WriteFile(filename1.c_str(), content1, &handler_));
  ASSERT_TRUE(file_system()->WriteFile(filename2.c_str(), content2, &handler_));
  int64 size;
