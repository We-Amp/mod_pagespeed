...
NgxUrlAsyncFetcher::NgxUrlAsyncFetcher(const char* proxy, ngx_log_t* log,
...
NgxUrlAsyncFetcher::~NgxUrlAsyncFetcher() {
  ...
  message_handler_->Message(
      kInfo, "Destruct NgxUrlAsyncFetcher with [%d] active fetchers",
...
bool NgxUrlAsyncFetcher::ParseUrl(ngx_url_t* url, ngx_pool_t* pool) {
  ...
  u_short port;
  if (ngx_strncasecmp(url->url.data,
                      reinterpret_cast<u_char*>(const_cast<char*>("http://")),
                      7) == 0) {
    scheme_offset = 7;
    port = 80;
  } else if (ngx_strncasecmp(
                 url->url.data,
                 reinterpret_cast<u_char*>(const_cast<char*>("https://")),
                 8) == 0) {
    scheme_offset = 8;
    port = 443;
  } else {
...
bool NgxUrlAsyncFetcher::Init(ngx_cycle_t* cycle) {
  log_ = cycle->log;
  CHECK(event_connection_ == nullptr) << "event connection already set";
  ...
  if (!ParseUrl(&proxy_, pool_)) {
    ngx_log_error(NGX_LOG_ERR, log_, 0,
                  "NgxUrlAsyncFetcher::Init parse proxy[%V] failed",
                  &proxy_.url);
    return false;
  }
  return true;
...
void NgxUrlAsyncFetcher::Fetch(const GoogleString& url,
  ...
  NgxFetch* fetch = new NgxFetch(url, async_fetch, message_handler, log_);
...
void NgxUrlAsyncFetcher::ReadCallback(const ps_event_data& data) {
  ...
  NgxUrlAsyncFetcher* fetcher =
      reinterpret_cast<NgxUrlAsyncFetcher*>(data.sender);

  fetcher->mutex_->Lock();
  fetcher->completed_fetches_.DeleteAll();

  for (Pool<NgxFetch>::iterator p = fetcher->pending_fetches_.begin(),
                                e = fetcher->pending_fetches_.end();
       p != e; p++) {
    NgxFetch* fetch = *p;
