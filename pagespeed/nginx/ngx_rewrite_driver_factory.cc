...
NgxRewriteDriverFactory::NgxRewriteDriverFactory(
  ...
  // Note: file_cache_clean_inode_limit was removed, CycloneCache handles cleanup
  default_options()->set_avoid_renaming_introspective_javascript(true);
...
UrlAsyncFetcher* NgxRewriteDriverFactory::AllocateFetcher(
  ...
  if (use_native_fetcher_) {
    ...
    // Use curl-based fetcher when not using the native nginx fetcher.
    // This provides reliable HTTP/HTTPS fetching as a fallback.
...
NamedLockManager* NgxRewriteDriverFactory::DefaultLockManager() {
  ...
  // ThreadSafeLockManager provides in-process thread coordination which
  // is appropriate for this deployment model.
