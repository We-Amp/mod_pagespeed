...
const GoogleString NgxConnection::ka_header = StrCat(
    "keep-alive ", Integer64ToString(NgxConnection::keepalive_timeout_ms));
    ...
    for (NgxConnectionPool::iterator p = connection_pool.begin();
      ...
      if (ngx_memn2cmp(static_cast<u_char*>(nc->sockaddr_),
        ...
        ngx_log_error(NGX_LOG_DEBUG, pc->log, 0,
                      "NgxFetch: re-using connection %p (pool size: %l)", nc,
...
void NgxConnection::Close() {
    ...
    for (NgxConnectionPool::iterator p = connection_pool.begin();
      ...
      if (*p == this) {
        ...
        ngx_log_error(NGX_LOG_DEBUG, c_->log, 0,
                      "NgxFetch: removed connection %p (pool size: %l)", this,
                      connection_pool.size());
  ...
  ngx_add_timer(c_->read,
                static_cast<ngx_msec_t>(NgxConnection::keepalive_timeout_ms));
...
NgxFetch::NgxFetch(const GoogleString& url, AsyncFetch* async_fetch,
...
NgxFetch::~NgxFetch() {
  if (timeout_event_ != nullptr && timeout_event_->timer_set) {
...
bool NgxFetch::Start(NgxUrlAsyncFetcher* fetcher) {
  ...
  bool ok = Init();
  if (ok) {
    ngx_log_error(NGX_LOG_DEBUG, log_, 0, "NgxFetch %p: initialized", this);
...
bool NgxFetch::Init() {
  ...
  timeout_event_ =
      static_cast<ngx_event_t*>(ngx_pcalloc(pool_, sizeof(ngx_event_t)));
  ...
  if (r_ == nullptr) {
    message_handler_->Message(kError, "NgxFetch: ngx_pcalloc failed for timer");
    return false;
  }
...
void NgxFetch::CallbackDone(bool success) {
  ngx_log_error(NGX_LOG_DEBUG, log_, 0, "NgxFetch %p: CallbackDone: %s", this,
                success ? "OK" : "FAIL");
  ...
  if (connection_ != nullptr) {
    ...
    if (success) {
      ...
      ngx_log_error(NGX_LOG_DEBUG, log_, 0,
                    "NgxFetch %p: connection %p attempt keep-alive: %s", this,
                    connection_, keepalive ? "Yes" : "No");
    }
...
void NgxFetch::ResolveDoneHandler(ngx_resolver_ctx_t* resolver_ctx) {
  ...
  if (resolver_ctx->state != NGX_OK) {
    if (fetch->timeout_event() != nullptr &&
        fetch->timeout_event()->timer_set) {
      ngx_del_timer(fetch->timeout_event());
      fetch->set_timeout_event(nullptr);
    }
  ...
  if (i == resolver_ctx->naddrs) {
    ...
    fetch->message_handler()->Message(
        ...
        static_cast<int>(resolver_ctx->name.len), resolver_ctx->name.data);
    fetch->CallbackDone(false);
    // CallbackDone() tore the fetch down (released the resolver ctx and ran the
    // completion callback). Must return here, like the resolver-error branch
    // above — otherwise execution falls through and reads
    // resolver_ctx->addrs[i] with i == naddrs (out-of-bounds, on a freed ctx),
    // and may call CallbackDone() a second time (LOG(FATAL)).
    return;
  }
  ...
  sin = reinterpret_cast<struct sockaddr_in*>(resolver_ctx->addrs[i].sockaddr);
...
int NgxFetch::InitRequest() {
  ...
  if (rc == NGX_AGAIN || rc == NGX_OK) {
    ...
    size = (method_len + 1 /* for the space */ + url_.uri.len +
            sizeof(" HTTP/1.0\r\n") - 1);
...
void NgxFetch::ConnectionReadHandler(ngx_event_t* rev) {
  ...
  while (rev->ready) {
    ...
    if (n == NGX_AGAIN) {
      ...
      ok = (fetch->content_length_known_ &&
            fetch->content_length_ == fetch->bytes_received_) ||
...
bool NgxFetch::HandleStatusLine(ngx_connection_t* c) {
  ...
  ngx_int_t n =
      ngx_http_parse_status_line(fetch->r_, fetch->in_, fetch->status_);
  if (n == NGX_ERROR) {  // parse status line error
    fetch->message_handler()->Message(kWarning,
                                      "NgxFetch: failed to parse status line");
    return false;
  } else if (n == NGX_AGAIN) {  // not completed
    return true;
  }

  ResponseHeaders* response_headers = fetch->async_fetch_->response_headers();
