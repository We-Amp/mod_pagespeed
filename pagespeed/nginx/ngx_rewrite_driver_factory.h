...
enum ProcessScriptVariablesMode { kOff, kLegacyRestricted, kAll };

class NgxRewriteDriverFactory : public SystemRewriteDriverFactory {
 public:
  // We take ownership of the thread system.
  explicit NgxRewriteDriverFactory(const ProcessContext& process_context,
  ...
  void set_resolver(ngx_resolver_t* resolver) { resolver_ = resolver; }
  void set_resolver_timeout(ngx_msec_t resolver_timeout) {
    resolver_timeout_ =
        resolver_timeout == NGX_CONF_UNSET_MSEC ? 1000 : resolver_timeout;
  }
  bool use_native_fetcher() { return use_native_fetcher_; }
  void set_use_native_fetcher(bool x) { use_native_fetcher_ = x; }
 ...
 private:
  ...
  using NgxMessageHandlerSet = std::set<NgxMessageHandler*>;
  ...
  NgxRewriteDriverFactory(const NgxRewriteDriverFactory&) = delete;
  NgxRewriteDriverFactory& operator=(const NgxRewriteDriverFactory&) = delete;
};
