...
bool SharedMemCache<kBlockSize>::InitCache(bool parent) {
  ...
  for (int s = 0; s < num_sectors_; ++s) {
    std::unique_ptr<Sector<kBlockSize>> sec =
        std::make_unique<Sector<kBlockSize>>(segment_.get(), s * sector_size,
                                             entries_per_sector_,
                                             blocks_per_sector_);
    bool ok;
    if (parent) {
...
GoogleString SharedMemCache<kBlockSize>::DumpStats() {
  ...
  return aggregate.Dump(static_cast<size_t>(entries_per_sector_) * num_sectors_,
                        static_cast<size_t>(blocks_per_sector_) * num_sectors_);
}
...
void SharedMemCache<kBlockSize>::ExtractPosition(
  ...
  static_assert(kAssociativity == 4, "need different code for other assoc");
