...
const char* const kGraphsVars[] = {
    ...
    "redis_async_deletes",
    "redis_blocking_hits",
    "redis_blocking_inserts",
    "redis_blocking_misses",
    "redis_blocking_deletes",
};

}  // namespace

StatisticsLogger::StatisticsLogger(int64 update_interval_ms,
                                   int64 max_logfile_size_kb,
                                   const StringPiece& logfile_name,
                                   MutexedScalar* last_dump_timestamp,
                                   MessageHandler* message_handler,
                                   Statistics* stats, FileSystem* file_system,
                                   Timer* timer)
  ...
  if (mutex->TryLock()) {
    if (current_time_ms >=
      ...
      FileSystem::OutputFile* statistics_log_file =
          file_system_->OpenOutputFileForAppend(logfile_name_.c_str(),
                                                message_handler_);
...
void StatisticsLogger::DumpConsoleVarsToWriter(int64 current_time_ms,
                                               Writer* writer) {
  writer->Write(absl::StrFormat("timestamp: %s\n",
                                Integer64ToString(current_time_ms).c_str()),
                message_handler_);
...
void StatisticsLogger::DumpJSON(bool dump_for_graphs,
                                const StringSet& var_titles, int64 start_time,
...
void StatisticsLogger::ParseDataFromReader(const StringSet& var_titles,
...
void StatisticsLogger::PrintJSON(const std::vector<int64>& list_of_timestamps,
...
bool StatisticsLogfileReader::ReadNextDataBlock(int64* timestamp,
  ...
  while (
    ...
    // buffer.
    size_t next_timestamp_pos =
        BufferFind("timestamp: ", offset + newline_pos + 1);
