...
test.describe("License page", () => {
  test("404 error displays without JavaScript errors", async ({ page }) => {
    const errors: string[] = [];
    page.on("pageerror", (err) => errors.push(err.message));
    ...
    // Take screenshot.
    await page.screenshot({
      path: "e2e/screenshots/license-page.png",
      fullPage: true,
    });

    // The license API returns 404. The page should handle it gracefully.
    // Check for either the new unavailable card or the old error paragraph.
    const unavailable = page.locator('[data-testid="license-unavailable"]');
    const errorPara = page.locator("p.error");

    const hasUnavailable = (await unavailable.count()) > 0;
    const hasError = (await errorPara.count()) > 0;

    // At least one error state should be visible.
    expect(hasUnavailable || hasError).toBe(true);

    if (hasUnavailable) {
      // New graceful error state.
      const title = page.locator(".unavailable-title");
      await expect(title).toHaveText("License Service Unavailable");

      const message = page.locator(".unavailable-message");
      const msgText = await message.textContent();
      expect(msgText).toContain("license status endpoint");

      const detail = page.locator(".unavailable-detail");
      const detailText = await detail.textContent();
      expect(detailText).toContain("404");

      await expect(page.locator("button", { hasText: "Retry" })).toBeVisible();
      await expect(
        page.locator("a", { hasText: "Purchase License" }),
      ).toBeVisible();
    }

    if (hasError) {
      // Old error state -- at least verify it mentions 404.
      const errorText = await errorPara.textContent();
  ...
  test("page layout is not broken", async ({ page }) => {
    await navigateTo(page, "#/license");
    await page.waitForTimeout(4000);
    ...
    // The h1 should say "License".
    const h1 = page.locator("h1");
    await expect(h1).toHaveText("License");

    // Refresh Status button should be visible.
    await expect(
      page.locator("button", { hasText: "Refresh Status" }),
    ).toBeVisible();

    // No raw HTML on the page.
    const bodyText = await page.locator(".page").textContent();
    expect(bodyText).not.toContain("<!doctype");
  });
