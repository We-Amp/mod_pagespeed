...
AprMemCache::~AprMemCache() { apr_pool_destroy(pool_); }
...
void AprMemCache::Get(const GoogleString& key, Callback* callback) {
  ...
  if (status == APR_SUCCESS) {
    ...
    if (status != APR_NOTFOUND) {
      ...
      apr_strerror(status, buf, sizeof(buf));
      message_handler_->Message(kError,
                                "AprMemCache::Get error: %s (%d) on key %s",
...
void AprMemCache::MultiGet(MultiGetRequest* request) {
  ...
  apr_status_t status =
      apr_memcache2_multgetp(memcached_, temp_pool, data_pool, hash_table);
  ...
  if (status == APR_SUCCESS) {
    for (int i = 0, n = request->size(); i < n; ++i) {
      ...
      if (status == APR_SUCCESS) {
        ...
        if (status != APR_NOTFOUND) {
          ...
          apr_strerror(status, buf, sizeof(buf));
          message_handler_->Message(
              kError, "AprMemCache::MultiGet error: %s (%d) on key %s", buf,
              status, key.c_str());
          if (status == APR_TIMEUP) {
            timeouts_->Add(1);
          }
    ...
    apr_strerror(status, buf, sizeof(buf));
    message_handler_->Message(kError,
                              "AprMemCache::MultiGet error: %s (%d) on %d keys",
...
void AprMemCache::PutHelper(const GoogleString& key,
  ...
  apr_status_t status = apr_memcache2_set(
      memcached_, hashed_key.c_str(), const_cast<char*>(key_and_value.data()),
      key_and_value.size(), 0, 0);
  if (status != APR_SUCCESS) {
    RecordError();
    char buf[kStackBufferSize];
    apr_strerror(status, buf, sizeof(buf));
    ...
    message_handler_->Message(
        kError, "AprMemCache::Put error: %s (%d) on key %s, value-size %d", buf,
        status, key.c_str(), value_size);
    if (status == APR_TIMEUP) {
      timeouts_->Add(1);
    }
  }
...
void AprMemCache::Put(const GoogleString& key, const SharedString& value) {
  ...
  if (key_value_codec::Encode(key, value, &key_and_value)) {
    ...
    message_handler_->Message(
        kError,
        "AprMemCache::Put error: key size %d too large, first "
...
void AprMemCache::Delete(const GoogleString& key) {
  ...
  if ((status != APR_SUCCESS) && (status != APR_NOTFOUND)) {
    ...
    apr_strerror(status, buf, sizeof(buf));
    message_handler_->Message(kError,
                              "AprMemCache::Delete error: %s (%d) on key %s",
                              buf, status, key.c_str());
    if (status == APR_TIMEUP) {
      timeouts_->Add(1);
    }
...
bool AprMemCache::GetStatus(GoogleString* buffer) {
  ...
  for (int i = 0, n = servers_.size(); i < n; ++i) {
    ...
    if (status == APR_SUCCESS) {
      ...
      StrAppend(buffer,
                "bytes:                 ", Integer64ToString(stats->bytes),
                "\n");
      StrAppend(buffer,
                "bytes_read:            ", Integer64ToString(stats->bytes_read),
                "\n");
      StrAppend(buffer, "bytes_written:         ",
                Integer64ToString(stats->bytes_written), "\n");
      StrAppend(buffer,
                "cmd_get:               ", IntegerToString(stats->cmd_get),
                "\n");
      StrAppend(buffer,
                "cmd_set:               ", IntegerToString(stats->cmd_set),
                "\n");
      ...
      StrAppend(buffer,
                "curr_items:            ", IntegerToString(stats->curr_items),
                "\n");
      StrAppend(buffer,
                "evictions:             ", Integer64ToString(stats->evictions),
                "\n");
      StrAppend(buffer,
                "get_hits:              ", IntegerToString(stats->get_hits),
                "\n");
      StrAppend(buffer,
                "get_misses:            ", IntegerToString(stats->get_misses),
                "\n");
      StrAppend(buffer, "limit_maxbytes:        ",
                IntegerToString(stats->limit_maxbytes), "\n");
      StrAppend(buffer,
                "pointer_size:          ", IntegerToString(stats->pointer_size),
                "\n");
      ...
      StrAppend(buffer,
                "threads:               ", IntegerToString(stats->threads),
                "\n");
      StrAppend(buffer, "total_connections:     ",
                IntegerToString(stats->total_connections), "\n");
      StrAppend(buffer,
                "total_items:           ", IntegerToString(stats->total_items),
                "\n");
      StrAppend(buffer, "\n");
...
void AprMemCache::ShutDown() { shutdown_.set_value(true); }
