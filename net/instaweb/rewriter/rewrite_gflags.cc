...
DEFINE_int64(css_outline_min_bytes, RewriteOptions::kDefaultCssOutlineMinBytes,
             "Number of bytes above which inline "
             "CSS resources will be outlined.");
DEFINE_int64(js_outline_min_bytes, RewriteOptions::kDefaultJsOutlineMinBytes,
             "Number of bytes above which inline "
...
DEFINE_int64(image_webp_timeout_ms, RewriteOptions::kDefaultImageWebpTimeoutMs,
...
DEFINE_int64(js_inline_max_bytes, RewriteOptions::kDefaultJsInlineMaxBytes,
             "Number of bytes below which javascript will be inlined.");
DEFINE_int64(css_flatten_max_bytes, RewriteOptions::kDefaultCssFlattenMaxBytes,
             "Number of bytes below which stylesheets will be flattened.");
DEFINE_int64(css_inline_max_bytes, RewriteOptions::kDefaultCssInlineMaxBytes,
...
DEFINE_bool(ajax_rewriting_enabled, false,
            "Deprecated. Use "
            "in_place_rewriting_enabled.");
DEFINE_bool(in_place_rewriting_enabled, false,
            "Boolean to indicate whether "
            "in-place-resource-optimization(IPRO) is enabled.");
DEFINE_bool(in_place_wait_for_optimized, false,
            "Indicates whether in-place "
...
DEFINE_bool(css_preserve_urls, false,
            "Boolean to indicate whether CSS URLS"
            "should be preserved.");
DEFINE_bool(js_preserve_urls, false,
            "Boolean to indicate whether JavaScript"
            "URLs should be preserved.");
...
DEFINE_bool(log_url_indices, false,
            "Log URL indices for every rewriter "
            "application.");
DEFINE_int64(max_html_cache_time_ms, RewriteOptions::kDefaultMaxHtmlCacheTimeMs,
...
DEFINE_bool(hide_referer_using_meta, false,
            "Hide referer by adding meta tag to the HTML.");
DEFINE_int64(
...
DEFINE_bool(proactively_freshen_user_facing_request, false,
            ...
            "expire in cache.");
DEFINE_bool(serve_stale_if_fetch_error, true,
            "Serve stale content if the "
            "fetch results in an error.");
DEFINE_int64(serve_stale_while_revalidate_threshold_sec, 0,
             "Threshold for "
...
DEFINE_int32(psa_flush_buffer_limit_bytes,
             ...
             "will be injected.");
DEFINE_int32(psa_idle_flush_time_ms, RewriteOptions::kDefaultIdleFlushTimeMs,
             "If the input HTML stops coming in for this many ms, a flush"
             " will be injected. Use a value <= 0 to disable.");
DEFINE_string(pagespeed_version, "",
              "Version number to put into X-Page-Speed "
              "response header.");
...
DEFINE_bool(lazyload_images_after_onload, false,
            "Boolean indicating whether "
            "lazyload images should load images when onload is fired. If "
            "false, images are loaded onscroll.");

DEFINE_bool(use_blank_image_for_inline_preview, false,
            "Boolean to decide "
...
DEFINE_bool(inline_only_critical_images, true,
            "Boolean indicating whether "
            "inline_images should inline only critical images or not.");

DEFINE_bool(critical_images_beacon_enabled, false,
            "If beaconing is enabled "
...
DEFINE_int64(implicit_cache_ttl_ms, RewriteOptions::kDefaultImplicitCacheTtlMs,
...
DEFINE_bool(enable_cache_purge, false,
            "Enables the Cache Purge API. This "
            "requires saving input URLs to each metadata cache entry to "
            "facilitate fast URL cache invalidation.");

DEFINE_bool(proactive_resource_freshening, false,
            "If set, the URLs of the "
...
DEFINE_bool(report_unload_time, false,
            "If enabled, sends beacons when page "
...
DEFINE_bool(
    enable_prioritizing_scripts, false,
...
DEFINE_int64(max_html_parse_bytes, RewriteOptions::kDefaultMaxHtmlParseBytes,
...
bool DomainMapRewriteDomain(DomainLawyer* lawyer, const StringPiece& to_domain,
                            const StringPiece& from, MessageHandler* handler) {
  return lawyer->AddRewriteDomainMapping(to_domain, from, handler);
}

bool DomainMapOriginDomain(DomainLawyer* lawyer, const StringPiece& to_domain,
                           const StringPiece& from, MessageHandler* handler) {
...
bool DomainAddShard(DomainLawyer* lawyer, const StringPiece& to_domain,
                    const StringPiece& from, MessageHandler* handler) {
  return lawyer->AddShard(to_domain, from, handler);
}

bool AddDomainMap(const StringPiece& flag_value, DomainLawyer* lawyer,
                  bool (*fn)(DomainLawyer* lawyer, const StringPiece& to_domain,
                             const StringPiece& from, MessageHandler* handler),
...
bool RewriteGflags::SetupOptionsOnly(RewriteOptions* options,
  ...
  if (WasExplicitlySet("image_max_rewrites_at_once")) {
    options->set_image_max_rewrites_at_once(FLAGS_image_max_rewrites_at_once);
  }
  ...
  if (WasExplicitlySet("image_recompress_quality")) {
    options->set_image_recompress_quality(FLAGS_image_recompress_quality);
  }
  ...
  if (WasExplicitlySet("image_webp_timeout_ms")) {
    options->set_image_webp_timeout_ms(FLAGS_image_webp_timeout_ms);
  }
  ...
  if (WasExplicitlySet("lazyload_images_blank_url")) {
    options->set_lazyload_images_blank_url(FLAGS_lazyload_images_blank_url);
  }
  ...
  if (WasExplicitlySet("inline_only_critical_images")) {
    options->set_inline_only_critical_images(FLAGS_inline_only_critical_images);
  }
  ...
  if (WasExplicitlySet("enable_prioritizing_scripts")) {
    options->set_enable_prioritizing_scripts(FLAGS_enable_prioritizing_scripts);
  }
  ...
  if (WasExplicitlySet("hide_referer_using_meta")) {
    options->set_hide_referer_using_meta(FLAGS_hide_referer_using_meta);
  }
  ...
  if (WasExplicitlySet("rewrite_domain_map")) {
    ret &=
        AddDomainMap(FLAGS_rewrite_domain_map, options->WriteableDomainLawyer(),
                     DomainMapRewriteDomain, handler);
  }

  if (WasExplicitlySet("shard_domain_map")) {
    ret &=
        AddDomainMap(FLAGS_shard_domain_map, options->WriteableDomainLawyer(),
                     DomainAddShard, handler);
  }

  if (WasExplicitlySet("origin_domain_map")) {
    ret &=
        AddDomainMap(FLAGS_origin_domain_map, options->WriteableDomainLawyer(),
  ...
  if (WasExplicitlySet("known_libraries")) {
    ...
    for (int max = library_specs.size() - 2; i < max; i += 3) {
      ...
      if (!options->RegisterLibrary(bytes, md5, url)) {
        LOG(ERROR) << "Invalid library md5 or url; skipping: " << md5 << " "
                   << url;
      }
      LOG(INFO) << "Registering library " << bytes << " " << md5 << " " << url;
    }
  ...
  if (WasExplicitlySet("experiment_specs")) {
    ...
    SplitStringPieceToVector(FLAGS_experiment_specs, "+", &experiment_specs,
                             true);
  ...
  ret &= SetRewriters("rewriters", FLAGS_rewriters.c_str(), "rewrite_level",
                      FLAGS_rewrite_level.c_str(), options, handler);
