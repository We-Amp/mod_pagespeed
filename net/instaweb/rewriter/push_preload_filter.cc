...
void PushPreloadFilter::StartDocumentImpl() {
  ...
  // Sort dependencies by order_key. Fonts are stored in a separate field
  // (see dependencies.proto), so merge them in before sorting; their order
  // keys were assigned in the same child key space as everything else, so
  // the sort interleaves them right after their parent stylesheet.
  ...
  for (const Dependency& dep : ordered_deps) {
    ...
    switch (dep.content_type()) {
      ...
      case DEP_FONT:
        ...
        StrAppend(&link_val, "; as=font; crossorigin");
        break;
      default:
